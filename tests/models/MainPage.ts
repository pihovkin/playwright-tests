...
export class MainPage {
  ...
  async checkElementsVisibility() {
    for (const { locator, name } of this.elements) {
      await test.step(`Перевірка відображення елемента ${name}`, async () => {
  ...
  async checkElementsText() {
    for (const { locator, name, text } of this.elements) {
      if (text) {
        await test.step(`Перевірка назви елемента ${name}`, async () => {
  ...
  async checkElementsHrefAttribute() {
    for (const { locator, name, attribute } of this.elements) {
      if (attribute) {
        await test.step(`Перевірка атрибутів ahref елемента ${name}`, async () => {
